    function output=model_SAC(t,X,flag_ode,Lambda,par,ISAC_factor,tstretch,trelax,timeprepace)
 switch nargin
        case 7 %do NOT perform dynamic stretch
            tstretch=-1;
        end
...
%parameters
BetaNs=par(1)*1E1; %stretch coefficient
BetaKo=par(5); %stretch coefficient
Gns=par(2)*1E-6*sigm_factor;
GKo=par(3)*1E-6; 
r=par(4);

%membrane capacitance
Cm=(((x)^4)/(0.035^4+(x)^4)+1)*(1.534*1E-4);

%SACns
GammaSLNs=BetaNs*((2*(x)^3)/(0.15^3+(x)^3))*0.25;
INsNa=(r*Gns*GammaSLNs*(v-ENa))/Cm;
INsK=(Gns*GammaSLNs*(v-EK))/Cm;
INs=(INsNa+INsK);
...
GKr = 0.0321 * sqrt(ko/5); % 1st element compensates for change to ko (sqrt(5/5.4)* 0.0362)
if celltype==1
    GKr=GKr*1.3;
elseif celltype==2
    GKr=GKr*0.8;
...
%update intracellular concentrations, using buffers for cai, cass, cajsr
    ...
    dnai=-(INa+INaL+3.0*INaCa_i+3.0*INaK+INab+INsNa)*Acap/(F*vmyo)+JdiffNa*vss/vmyo;

    dnass=-(ICaNa+3.0*INaCa_ss)*Acap/(F*vss)-JdiffNa;

    dki=-(ICaK_i+Ito+IKr+IKs+IK1+IKb+Istim-2.0*INaK+INsK+IKo)*Acap/(F*vmyo)+JdiffK*vss/vmyo;
